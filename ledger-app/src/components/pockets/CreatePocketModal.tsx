...
export default function CreatePocketModal({ onClose }: CreatePocketModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.name.trim() || isNaN(target) || target <= 0 || isNaN(contrib) || contrib <= 0) return;
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-3.5 overflow-y-auto">
          ...
          <div className="space-y-1">
            ...
            <p className="text-[10px] text-slate-400">
              * Note: Completion dates are calculated dynamically using your actual forecast surplus.
            </p>
          </div>
